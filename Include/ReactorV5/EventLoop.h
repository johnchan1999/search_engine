...
namespace OurPool
{
    ...
    public:
        ...
        void loop();                   // 等待事件，分配任务
        void unloop();                 // 结束事件循环
        void runInLoop(Functors &&cb); // 传递任务到此函数
    ...
    private:
        void waitEpollFd();          // 执行epoll_wait的函数
        void handleNewConnection();  // 处理新的连接
        void handleMessage(int fd);  // 处理消息的发送
        int createEpollFd();         // 创建文件描述符epfd
        void addEpollReadFd(int fd); // 将文件描述符放在红黑树上进行监听
        void delEpollReadFd(int fd); // 将文件描述符从红黑树上删除

    private:
        void handleRead();        // 解决读事件
        void wakeup();            // 唤醒线程执行任务
